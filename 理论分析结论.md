...
flowchart TD
    ...
    E --> A[Attention: GDN/Linear ×30 或 Full Attention ×10]
    A --> O[Attention 输出投影 GEMM: out_proj/o_proj]
    O --> C1{C1: TP AllReduce<br/>或 FlashComm RS/MMRS}
    C1 --> N[Residual + Post-Attention RMSNorm]
    N --> R[MoE Router + Top-K=8]
    R --> Q[动态量化/Token Dispatch<br/>C3: EP/AllGather/AlltoAll/MC2]
    Q --> G[GMM1 + SwiGLU + GMM2 + Token Combine<br/>C4: Routed Experts]
    N --> S[Shared Expert gate/up + SwiGLU + down<br/>C5: 可选独立 Stream]
    G --> M[Merge Routed + Shared]
    S --> M
    M --> C6{C6: MoE 最终 TP/EP 规约<br/>AllReduce/ReduceScatter/融合 Combine}
    C6 --> L[Residual → 下一 Decoder Layer ×40]
    L --> A
    L --> F[完成第 40 层后 Final RMSNorm]
    F --> MT[MTP 可选：1 层预测/验证分支]
